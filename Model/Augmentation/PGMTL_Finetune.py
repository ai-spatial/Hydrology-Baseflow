...
l1 = tf.get_variable('l1',[1],  tf.float32,
                                  initializer=tf.constant_initializer(0.1)) #default 1

l2 = tf.get_variable('l2',[1],  tf.float32,
                                  initializer=tf.constant_initializer(0.1)) #default 1

# l3 = tf.get_variable('l3',[1],  tf.float32,
#                                   initializer=tf.constant_initializer(0.0)) #default 0.1
...
cost_val1 = loss_measure(pred_tr,y_val1,m_val1) 
cost_val2 = loss_measure(pred_tr,y_val2,m_val2) 


cost = cost_tr1 + l1 * cost_val1 + l2*cost_val2
...
grads = tf.gradients(cost, tvars[:-2])

optimizer = tf.train.AdamOptimizer(learning_rate)
train_op = optimizer.apply_gradients(zip(grads, tvars[:-2]))
...
for ct in range(k):
    ...
    m_tea = maska[loc,cv_idx*3600:(cv_idx+1)*3600]
    m_teb = maskb[loc,cv_idx*3600:(cv_idx+1)*3600]
    ...
    if cv_idx==2:
        ...
        m_tr_1a = maska[loc,:3600]
        m_tr_1b = maskb[loc,:3600]
        ...
        m_tr_2a = maska[loc,3600:2*3600:]
        m1_tr_2a = maska1[loc,3600:2*3600:]
        m_tr_2b = maskb[loc,3600:2*3600:]
        m1_tr_2b = maskb1[loc,3600:2*3600:]
    ...
    #saver.restore(sess, "./step2/pretain" + str(ct) + ".ckpt") 
    vars1 = sess.run(tvars[12:])
    print(vars1)
    ...
    for epoch in range(epochs):
        ...
        for i in range(N_sec): # better code?
            ...
            batch_ma = m_train_1a[index,:]
            batch_mb = m_train_1b[index,:]
        ...
        print('Epoch '+str(epoch)+' station No.' + str(ct) +' batch 1: loss '+"{:.4f}".format(alos[0]/N_sec) )
        ...
        for i in range(N_sec): # better code?
            ...
            batch_ma = m_train_2a[index,:]
            batch_mb = m_train_2b[index,:]
            if np.sum(batch_m)>0:
                _, los = sess.run(
                    [train_op, cost],
                    feed_dict = {
        ...
        print('Epoch '+str(epoch)+' station No.' + str(ct) +' batch 2: loss '+"{:.4f}".format(alos[0]/N_sec) )
        #print(epoch, alos/N_sec)
        ...
        for i in range(N_sec): # better code?
            ...
            batch_ma = m_testa[index,:]
            batch_mb = m_testb[index,:]
