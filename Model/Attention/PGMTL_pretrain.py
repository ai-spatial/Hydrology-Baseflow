...
#new parameter training for assmable


Wi1 = tf.get_variable('Wi1',[state_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))

Wf1 = tf.get_variable('Wf1',[state_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))

Wo1 = tf.get_variable('Wo1',[state_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))

Wg1 = tf.get_variable('Wg1',[state_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))


Ui1 = tf.get_variable('Ui1',[input_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))

Uf1 = tf.get_variable('Uf1',[input_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))

Uo1 = tf.get_variable('Uo1',[input_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))

Ug1 = tf.get_variable('Ug1',[input_size, state_size], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))

w_fin1 = tf.get_variable('w_fin1',[state_size, n_classes], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))
b_fin1 = tf.get_variable('b_fin1',[n_classes],  tf.float32,
                                  initializer=tf.constant_initializer(0.0))


w_fin2 = tf.get_variable('w_fin2',[state_size, n_classes], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))
b_fin2 = tf.get_variable('b_fin2',[n_classes],  tf.float32,
                                  initializer=tf.constant_initializer(0.0))


w_fin3 = tf.get_variable('w_fin3',[state_size, n_classes], tf.float32,
                                  tf.random_normal_initializer(stddev=0.02))
b_fin3 = tf.get_variable('b_fin3',[n_classes],  tf.float32,
                                  initializer=tf.constant_initializer(0.0))


def forward_w(xi, gx, W1,b1,Wi,Wf,Wo,Wg,Ui,Uf,Uo,Ug,w_fin,b_fin,w_fin1,b_fin1,w_fin2,b_fin2,reuse=False):
...
#    f = tf.sigmoid(tf.matmul(x[:,0,:],Uf))
    ...
    w_ml = tf.matmul(oh,w_fin)+b_fin
    w_ml = tf.reshape(w_ml,[-1,n_steps,1])
    
    w_val1 = tf.matmul(oh,w_fin1)+b_fin1
    w_val1 = tf.reshape(w_val1,[-1,n_steps,1])
    
    w_val2 = tf.matmul(oh,w_fin2)+b_fin2
    w_val2 = tf.reshape(w_val2,[-1,n_steps,1])
    
    w_all = tf.concat([w_ml, w_val1, w_val2], 2) 
    weight = tf.nn.softmax(w_all)
...
h_tr,pred = forward(x_tr1, gx_tr1, W1,b1,Wi,Wf,Wo,Wg,Ui,Uf,Uo,Ug,w_fin,b_fin)

w_ml,w_val1,w_val2 = forward_w(x_tr1, gx_tr1, W1,b1,Wi1,Wf1,Wo1,Wg1,Ui1,Uf1,Uo1,Ug1,w_fin1,b_fin1,w_fin2,b_fin2,w_fin3,b_fin3,reuse=False)
pred_tr = w_ml*pred + w_val1*y_val1 + w_val2*y_val2
...
grads = tf.gradients(cost, tvars[:12])
...
for it in range(1,n_upd):
    _,pred1 = forward(x_tr1, gx_tr1, W1n,b1n,Win,Wfn,Won,Wgn,Uin,Ufn,Uon,Ugn,w_finn,b_finn)
    w_ml,w_val1,w_val2 = forward_w(x_tr1, gx_tr1, W1n,b1n,Wi1,Wf1,Wo1,Wg1,Ui1,Uf1,Uo1,Ug1,w_fin1,b_fin1,w_fin2,b_fin2,w_fin3,b_fin3,reuse=False)
    pred1_tr = w_ml*pred1 + w_val1*y_val1 + w_val2*y_val2
    cost = loss_measure(pred1_tr,y_tr1,m_tr1) 
...
_,pred2 = forward(x_tr2, gx_tr2, W1n,b1n,Win,Wfn,Won,Wgn,Uin,Ufn,Uon,Ugn,w_finn,b_finn)
w_ml,w_val1,w_val2 = forward_w(x_tr2, gx_tr2, W1n,b1n,Wi1,Wf1,Wo1,Wg1,Ui1,Uf1,Uo1,Ug1,w_fin1,b_fin1,w_fin2,b_fin2,w_fin3,b_fin3,reuse=False)
pred2_tr = w_ml*pred2 + w_val1*y_val1_2 + w_val2*y_val2_2
...
for r in range(recycle):
    ...
    for ct in k_list:
        ...
        if ct == k_list[0] and r == 0:
            ...
            vars_op = tf.variables_initializer(tvars[12:])
            sess.run(vars_op)       
            saver1 = tf.train.Saver(max_to_keep=3)
        ...
        for epoch in range(epochs):
            ...
            for i in range(N_sec): # better code?
                ...
                batch_ya1 = y_train_2a[index,:,:]
                batch_yb1 = y_train_2b[index,:,:]
            ...
            for i in range(N_sec): # better code?
                ...
                batch_ya1 = y_testa[index,:]
                batch_yb1 = y_testb[index,:]
