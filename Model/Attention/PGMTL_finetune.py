...
''' Declare constants '''
learning_rate = 0.0005#0.001#0.005/3/2 default 0.002#0.0009
...
h_tr,pred = forward(x_tr, gx_tr, W1,b1,Wi,Wf,Wo,Wg,Ui,Uf,Uo,Ug,w_fin,b_fin)

w_ml,w_val1,w_val2 = forward_w(x_tr, gx_tr, W1,b1,Wi1,Wf1,Wo1,Wg1,Ui1,Uf1,Uo1,Ug1,w_fin1,b_fin1,w_fin2,b_fin2,w_fin3,b_fin3,reuse=False)
...
cluster = np.load('./step2/cluster.npy')


pred_all = np.zeros((len(gage_feat),3600))
for ct in range(k):
    print('cluster No.', ct)
    ...
    saver.restore(sess, "./step2/pretrain.ckpt") 

    saver1 = tf.train.Saver(var_list=tvars[12:]) 
    saver1.restore(sess, "./step2/pretain" + str(ct) + ".ckpt") 
    ...
    for epoch in range(epochs):
        ...
        for i in range(N_sec): # better code?
            ...
            batch_ya = y_train_2a[index,:]
            batch_yb = y_train_2b[index,:]
        ...
        print('Epoch '+str(epoch)+' station No.' + str(ct) +' batch 2: loss '+"{:.4f}".format(alos/N_sec) )
        #print(epoch, alos/N_sec)
        ...
        #print('parameter tuning: ', W2,W3,B2,B3)    
    ...
    save_path = saver2.save(sess, "./step3/finetune" + str(ct) + ".ckpt") 
    print("Model saved in path: %s" % save_path)          
np.save('./step3/pred_finetune.npy',pred_all)   